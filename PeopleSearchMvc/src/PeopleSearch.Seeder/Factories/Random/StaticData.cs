...
namespace PeopleSearch.Seeder.Factories.Random
{
    ...
    {
        public static string[] RandomInterests(int seed)
        {
            var random = new System.Random(seed);
            var numberOfInterests = random.Next(0, 10);
            var result = new Dictionary<int, string>();
            while (result.Count < numberOfInterests)
            {
                var randomIndex = random.Next(0, Interests.Length);
                if (result.ContainsKey(randomIndex) == false)
                {
                    result.Add(randomIndex, Interests[randomIndex]);
                }
            }
